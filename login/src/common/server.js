...
// Determine which environment to use based on ENV_STATE
const envState = process.env.ENV_STATE || "stable";
console.log(`Starting server in ${envState} mode`);

// Import the appropriate implementation
let implementation;
if (envState === "dev") {
  implementation = require("../dev/implementation");
} else {
  implementation = require("../stable/implementation");
}
...
function createServer(config) {
    ...
    res.sendFile(indexPath, (err) => {
      if (err) {
        ...
        if (!res.headersSent) {
          res
            .status(404)
            .send("Error: index.html not found at determined path.");
        }
      }
    });
  });
  ...
  app.post("/login", async (req, res) => {
    ...
    const validationResult = implementation.validatePassword(password);

    if (!validationResult.isValid) {
      console.log(
        `Validation failed for user ${username}: ${validationResult.message}`
      );
      return res.redirect(
        "/?error=" + encodeURIComponent(validationResult.message)
      );
    }

    console.log(
      `Validation passed for user ${username}. Proceeding with login query.`
    );

    try {
      // Execute login query using the configured executor
      const result = await implementation.executeLoginQuery(
        pool,
        username,
        password
      );
      console.log(
        `Query returned ${result.rows ? result.rows.length : 0} rows.`
      );

      if (result.rows && result.rows.length > 0) {
        // Handle successful login using the configured handler
        implementation.handleLoginSuccess(req, res, result);
      } else {
...
// Create the server using the selected implementation
const { app } = createServer(implementation);

// Start the server
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`Server running on port ${PORT} in ${envState} mode`);
});
...
//         password: process.env.POSTGRES_PASSWORD,
//         port: process.env.DB_PORT,
//         env_state: process.env.ENV_STATE,
//     });
